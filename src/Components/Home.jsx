...
import { BookText,BookOpen,Calculator,Landmark,Beaker,Atom,Globe,Monitor,LockKeyhole,ChevronRight, AlertTriangle, X, Info, Crown, Star, Shield, User, LogOut, Lock, Sparkles, Zap, TrendingUp, Rocket  } from 'lucide-react';
...
export function Home() {
  ...
  useEffect(() => {
    const checkUserSession = () => {
      try {
        ...
        if (!userData) {
          // Si no hay datos de usuario, activar modo invitado
          setIsGuest(true);
          setUser(null);
        } else {
  ...
  const handleSubjectClick = (subject) => {
    if (isGuest) {
      // Si es invitado, mostrar modal pidiendo login
      setLockedSubject({ ...subject, guestMode: true });
    } else if (subject.locked) {
      setLockedSubject(subject);
    } else {
      // Redirigir a la página de asignaturas o donde corresponda
  ...
  const getWelcomeMessage = () => {
    ...
    // Mapeo basado en tu LoginController: idrol 1 = admin, idrol 2 = usuario
    switch (user.idrol) {
      case 1:
        return {
      ...
      case 3: // Asumiendo que 3 sería usuariopluss
        return {
          text: `Bienvenido ${user.name} 🎉`,
          icon: <Crown size={24} />,
  ...
  // MODO INVITADO - Vista para usuarios no logueados
  if (isGuest) {
    return (
      <HomeContainer>
        <Header>
        ...
        <Main>
          {/* Banner de Bienvenida para Invitados */}
          <GuestWelcomeBanner>
            <GuestBannerContent>
              <GuestBannerIcon>
                <Sparkles size={40} />
              ...
              <GuestBannerText>
                <GuestBannerTitle>¡Bienvenido a Knowly!</GuestBannerTitle>
                <GuestBannerSubtitle>
                  Descubre un mundo de aprendizaje interactivo. 
                  <br />
                  <strong>Registrate</strong> para acceder a todas las materias.
                </GuestBannerSubtitle>
              </GuestBannerText>
              <GuestBannerButton onClick={() => navigate('/registro')}>
                <Zap size={20} />
                Comenzar Ahora
              </GuestBannerButton>
            </GuestBannerContent>
          ...
          {subjectCategories.map((category, categoryIndex) => (
            <React.Fragment key={categoryIndex}>
              <CategorySection>
                ...
                <SubjectsGrid>
                  {category.subjects.map((subject, index) => (
                    <SubjectCard key={index} isGuest={true}>
                      <GuestOverlay>
                        <GuestLockIcon>
                          <Lock size={32} />
                        </GuestLockIcon>
                        <GuestLockText>Inicia sesión para acceder</GuestLockText>
                      </GuestOverlay>
                      <SubjectContent>
              ...
              {/* Banners después de "Saberes y Pensamiento Cientifico" */}
              {category.category === "Saberes y Pensamiento Cientifico" && (
                <MidSectionAds>
                  ...
                  <AdBannerHorizontal gradient="linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%)">
                    ...
                    <AdBannerButton onClick={() => navigate('/registro')}>
                      Registrarse Gratis
                    </AdBannerButton>
                  </AdBannerHorizontal>
                </MidSectionAds>
        ...
        {/* Banners de pie de página - Ahora son DOS */}
        <FooterAds>
        ...
        {lockedSubject && lockedSubject.guestMode && (
          <ModalOverlay onClick={closeModal}>
            <ModalContent onClick={(e) => e.stopPropagation()}>
              ...
              <ModalBody>
                <p>
                  Para acceder a <strong>{lockedSubject.name}</strong> y todas las materias,
              ...
              <ModalFooter>
                ...
                <ConfirmButton onClick={() => navigate('/Login')}>
                  Iniciar Sesión
                </ConfirmButton>
              </ModalFooter>
            </ModalContent>
...
const GuestWelcomeBanner = styled.div`
  background: linear-gradient(135deg, #7c3aed 0%, #a855f7 100%);
  border-radius: 24px;
  padding: 3rem 2rem;
  margin-bottom: 3rem;
...
const GuestBannerTitle = styled.h1`
  font-size: 2.5rem;
  font-weight: 800;
  margin: 0 0 0.5rem 0;
  letter-spacing: 1px;
...
const GuestLockIcon = styled.div`
  color: white;
...
// NUEVOS STYLED COMPONENTS PARA BANNERS INTERMEDIOS
...
// NUEVOS STYLED COMPONENTS PARA FOOTER - DOS BANNERS
